...
//int CountTwoDigitNumbers(int[] arr, int start, int end){    int count = 0;    for (int i = 0; i < arr.Length; i++)    {        if (arr[i] >= start && arr[i] <= end) count++;    }    return count;}Console.WriteLine("Введите начало диапазона");int startNumber = Convert.ToInt32(Console.ReadLine());Console.WriteLine("Введите конец диапазона");int endNumber = Convert.ToInt32(Console.ReadLine());int[] array = CreateArrayRndInt(10, -1000, 1000);PrintArray(array);int result = CountTwoDigitNumbers(array, startNumber, endNumber);Console.WriteLine($" -> {result}");

int[] CreateArrayRndInt(int size, int min, int max)//случайные элементы массива
{
